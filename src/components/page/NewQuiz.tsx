...
import { usePollinationsChat } from "@pollinations/react"; // Import Pollinations AI hook

export default function QuizCreator() {
  const [quizInfo, setQuizInfo] = useState({
    id: uuidv4(), // Unique ID for the quiz
    title: "",
    description: "",
  ...
  const [currentQuestion, setCurrentQuestion] = useState({
    ...
    explanation: "",
    image: "", // Base64 string for the question image
    answers: [],
  });
  ...
  // State for AI-generated quiz
  const [aiQuizFormVisible, setAiQuizFormVisible] = useState(false);
  const [aiQuizForm, setAiQuizForm] = useState({
    topic: "",
    keywords: "",
    numQuestions: 5,
    numAnswers: 4,
    questionType: "both", // true_false, multiple_choice, or both
    timeLimit: 10,
    randomizeQuestions: true,
  ...
  const handleAiQuizFormSubmit = async (e) => {
    e.preventDefault();
    setIsGeneratingQuiz(true);

    // Construct the prompt for the AI
    const prompt = `Create a quiz with the following details:
- Topic: ${aiQuizForm.topic}
- Keywords: ${aiQuizForm.keywords}
- Number of Questions: ${aiQuizForm.numQuestions}
- Number of Answers per Question: ${aiQuizForm.numAnswers}
- Question Types: ${aiQuizForm.questionType}
- Time Limit: ${aiQuizForm.timeLimit} minutes
- Randomize Questions: ${aiQuizForm.randomizeQuestions}
- Randomize Answers: ${aiQuizForm.randomizeAnswers}
...
Ensure the response contains only the JSON object and no additional text.`;
  ...
  useEffect(() => {
    if (messages.length > 0 && !isLoading) {
      ...
      try {
        ...
        const generatedQuiz = JSON.parse(lastMessage);
        ...
        const quizWithIds = {
          ...
          questions: generatedQuiz.questions.map((question) => ({
            ...question,
            id: uuidv4(), // Add unique ID for the question
            answers: question.answers.map((answer) => ({
              ...answer,
              id: uuidv4(), // Add unique ID for the answer
            })),
          })),
        };

        // Update the quiz info and questions
        setQuizInfo({
          ...quizInfo,
          title: quizWithIds.title,
          description: quizWithIds.description,
          randomizeQuestions: quizWithIds.randomizeQuestions,
          randomizeAnswers: quizWithIds.randomizeAnswers,
          timeLimit: quizWithIds.timeLimit,
        });

        setQuestions(quizWithIds.questions);

        // Stop loading only after the quiz is fully rendered
        setIsGeneratingQuiz(false);
        setAiQuizFormVisible(false); // Hide the AI form
      } catch (error) {
        console.error("Failed to parse AI response:", error);
        setIsGeneratingQuiz(false); // Stop loading on error
      }
    }
  }, [messages, isLoading]);
  ...
  const validateQuestion = () => {
    ...
    if (currentQuestion.answers.length < 2 || currentQuestion.answers.length > 6) {
      newErrors.answers = "A question must have between 2 and 6 answers.";
      isValid = false;
  ...
  const handleSaveAnswer = () => {
    if (validateAnswer() && currentQuestion.answers.length < 6) {
  ...
  const handleSaveQuiz = () => {
    ...
    const savedQuizzes = JSON.parse(localStorage.getItem("quizzes")) || []; // Get existing quizzes
    savedQuizzes.push(quiz); // Add the new quiz
    localStorage.setItem("quizzes", JSON.stringify(savedQuizzes)); // Save back to localStorage
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      >
        <CardHeader>
          ...
          <CardDescription>Create a quiz automatically using AI.</CardDescription>
        </CardHeader>
        <CardContent>
          {!aiQuizFormVisible ? (
            <Button onClick={() => setAiQuizFormVisible(true)} className="w-full">
              Create Quiz with AI
            </Button>
          ) : (
            <form onSubmit={handleAiQuizFormSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="topic">Topic</Label>
                <Input
                  id="topic"
                  placeholder="Enter quiz topic"
                  value={aiQuizForm.topic}
                  onChange={(e) => setAiQuizForm({ ...aiQuizForm, topic: e.target.value })}
                  required
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={aiQuizForm.keywords}
                  onChange={(e) => setAiQuizForm({ ...aiQuizForm, keywords: e.target.value })}
                  disabled={isGeneratingQuiz}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={aiQuizForm.numQuestions}
                  onChange={(e) =>
                    setAiQuizForm({ ...aiQuizForm, numQuestions: parseInt(e.target.value) })
                  }
                  min={1}
                  required
                  disabled={isGeneratingQuiz}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  value={aiQuizForm.questionType}
                  onChange={(e) =>
                    setAiQuizForm({ ...aiQuizForm, questionType: e.target.value })
                  }
                  disabled={isGeneratingQuiz}
                >
                  <option value="both">Both (True/False and Multiple Choice)</option>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={aiQuizForm.timeLimit}
                  onChange={(e) =>
                    setAiQuizForm({ ...aiQuizForm, timeLimit: parseInt(e.target.value) })
                  }
                  min={1}
                  required
                  disabled={isGeneratingQuiz}
              ...
              <div className="flex items-center space-x-4">
                <Switch
                  ...
                  checked={aiQuizForm.randomizeQuestions}
                  onCheckedChange={(checked) =>
                    setAiQuizForm({ ...aiQuizForm, randomizeQuestions: checked })
                  }
                  disabled={isGeneratingQuiz}
                />
              ...
              <div className="flex items-center space-x-4">
                <Switch
                  id="randomizeAnswers"
                  checked={aiQuizForm.randomizeAnswers}
                  onCheckedChange={(checked) =>
                    setAiQuizForm({ ...aiQuizForm, randomizeAnswers: checked })
                  }
                  disabled={isGeneratingQuiz}
                />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <CardHeader>
            ...
            <CardDescription>Add details about your quiz.</CardDescription>
            ...
            <div className="space-y-2">
              ...
              {quizInfo.image && (
                <div className="mt-2">
                  ...
                  <Button variant="destructive" size="sm" onClick={handleRemoveQuizImage} className="mt-2" disabled={isGeneratingQuiz}>
                    Remove Image
                  </Button>
                </div>
              )}
        ...
        {!isAddingQuestion && (
          ...
          >
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleAddQuestion} className="w-full" disabled={isGeneratingQuiz}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Question
              </Button>
            </CardContent>
            <CardFooter className="flex justify-end gap-2">
              <Button variant="outline" onClick={handleCancelQuiz} disabled={isGeneratingQuiz}>
                Cancel
              </Button>
              <Button onClick={handleSaveQuiz} disabled={isGeneratingQuiz}>
                Save Quiz
              </Button>
            </CardFooter>
          </MagicCard>
        ...
        {isAddingQuestion && (
          ...
          >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {currentQuestion.image && (
                  <div className="mt-2">
                    ...
                    <Button variant="destructive" size="sm" onClick={handleRemoveImage} className="mt-2" disabled={isGeneratingQuiz}>
                      Remove Image
                    </Button>
                  </div>
                )}
              ...
              <div className="space-y-4">
                ...
                {currentQuestion.type === "multiple_choice" && !isAddingAnswer && currentQuestion.answers.length < 5 && (
                  <Button onClick={handleAddAnswer} className="w-full" disabled={isGeneratingQuiz}>
                ...
                {isAddingAnswer && (
                  ...
                  >
                    ...
                    <CardFooter className="flex justify-end gap-2">
                      <Button variant="outline" onClick={handleCancelAnswer} disabled={isGeneratingQuiz}>
                        Cancel
                      </Button>
                      <Button onClick={handleSaveAnswer} disabled={isGeneratingQuiz}>
                        Save Answer
                      </Button>
                    </CardFooter>
                  </MagicCard>
            ...
            <CardFooter className="flex justify-end gap-2">
              <Button variant="outline" onClick={handleCancelQuestion} disabled={isGeneratingQuiz}>
                Cancel
              </Button>
              <Button onClick={handleSaveQuestion} disabled={isGeneratingQuiz}>
